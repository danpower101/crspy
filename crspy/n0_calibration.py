...
#  NO calibration - standalone function
...
def n0_calib_standalone(country="NON", sitenum="000", sensordata=None, calibdata=None, meta=None, defineaccuracy=0.01, bd=None, soc=None, lw=None):
    ...
    try:
        ...
        sitename = str(country)+"_SITE_"+str(sitenum)
    if soc == None and meta == None:
        print("We need an SOC reading. Please provide one and try again.")
        return
    elif soc == None and meta != None:
        soc = meta.loc[(meta.COUNTRY == country) & (
            meta.SITENUM == sitenum), 'SOC'].item()
    if lw == None and meta == None:
        print("We need a lattice water reading. Please provide one and try again.")
        return
    elif lw == None and meta != None:
        lw = meta.loc[(meta.COUNTRY == country) & (
            meta.SITENUM == sitenum), 'LW'].item()
    Hveg = 0  # Hveg not used due to lack of reliable data and small impact.
    ...
    if isinstance(calibdata, pd.DataFrame):
        df = calibdata
        COSMOScols = ['label:number', 'type:text', 'uri:url', 'change:text',
                    'changedItem:text', 'modified:text', 'Depth_cm:number',
                    'Wet_total_g:number', 'Dry_total_g:number', 'Tare_g:number',
    ...
    if "LOC" in df.columns:
        ...
        try:
            ...
            print("Please check your LOC_rad column")
    ...
    if "DEPTH_AVG" not in df.columns:
        ...
        try:
            ...
            print("Please check your DEPTH_AVG column")
    ...
    with np.errstate(divide='ignore'):  # prevent divide by 0 error message
        for i in range(numdays):
            ...
            reler['N0'] = range(int(n_max/2), int(n_max*2))  # Add N0 for csv write
    ...
    totalerror['N0'] = range(int(n_max/2), int(n_max*2))
    # Create object that maintains the index value at min
    minindex = totalerror.loc[totalerror.RelErr == minimum_error]
    print("Done")
    # Show the minimum error value with the index valu. Index = N0
